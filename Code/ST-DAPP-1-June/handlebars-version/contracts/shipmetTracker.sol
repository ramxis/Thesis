...
contract ShipmentTracker {
    ...
    event ShimpentTrackingEvent(address,int,string,string);//TODO:remove
    event LogRequirements(string _reqID,string _title,uint _severityLevel,int _minThreshold,int _maxThreshold,bool _minFlag,bool _maxFlag);
    event NewTrackingNumber(string trackingNr);
    event ShippingEvent(address,int,string,string,string,uint,uint);
    ...
    */
    function GetRequirementObject(string _reqID) public constant onlyAuthorized returns(string _rreqID,string _title,uint _severityLevel,int _minThreshold,int _maxThreshold,bool _minFlag,bool _maxFlag) {
      return (RequirementsList[_reqID].reqID,RequirementsList[_reqID].title,RequirementsList[_reqID].severityLevel,RequirementsList[_reqID].minThreshold,RequirementsList[_reqID].maxThreshold,RequirementsList[_reqID].minFlag,RequirementsList[_reqID].maxFlag);
    }
    ...
    /**
      ...
      callebale by: only the contract publisher
    */
    function SetRequirements(string _reqID, string _requireTitle, uint _level, int _minthreshold, int _maxthreshold,bool _minFlag,bool _maxFlag) public onlyOwner {
      ...
      emit LogRequirements(_reqID,_requireTitle,_level,_minthreshold, _maxthreshold,_minFlag,_maxFlag);
    }
    ...
    */
    function ChkViolations(ShipmentData data,string _trackingNr) internal {
        if(keccak256(data.dataID)==keccak256(RequirementsList[data.dataID].reqID)) {
          ...
          if((RequirementsList[data.dataID].minFlag)&&(RequirementsList[data.dataID].maxFlag)) {
            if((RequirementsList[data.dataID].minThreshold<data.sensorData)&&(data.sensorData<RequirementsList[data.dataID].maxThreshold)) {
              return;//_;
            }
            else {
              emit ReqViolationEventMinMax(data.shipperAddr,
                                          _trackingNr,
                                          data.dataID,
                                          RequirementsList[data.dataID].title,
                                          RequirementsList[data.dataID].severityLevel,
                                          data.sensorData,
                                          data.currentLocation,
                                          RequirementsList[data.dataID].minThreshold,
                                          RequirementsList[data.dataID].maxThreshold,
                                          data.dataBlockNr,
                                          data.dataTimestamp);
            }
          }
          else if((!RequirementsList[data.dataID].minFlag)&&(RequirementsList[data.dataID].maxFlag)){
            if(data.sensorData<RequirementsList[data.dataID].maxThreshold) {
              return;//_;
            }
            else {
          ...
          if(data.sensorData>RequirementsList[data.dataID].minThreshold) {
            return;//_;
          }
          else {
    ...
    */
    ...
    /**
      ...
      callebale by: only the contract publisher
    */
    function chkViolationByID(string _trackingNr) constant public onlyAuthorized {
      for(uint i=0;i<4;i++) {
        //cannot use block.timestamp as miners are allowed to modify it by +- 900 secs
        //emit RequirementViolated("tarckingNR",_dataID,"temp",i,block.number,10*i);
      }
    ...
    */
    ...
    function LogTrackingInformation(int data, string dataID, string location, string trackingNr) public onlyAuthorized  {
      ...
      ShipmentData memory trackingData = ShipmentData(msg.sender,data,dataID,location,block.number,block.timestamp);
      ...
      emit ShippingEvent(msg.sender,data,dataID,location,trackingNr,block.number,block.timestamp);
    ...
    */
    ...
    /**
      ...
      callebale by: only authorized handlers
    */
    function GetLoggedData(uint index,string trackingNr) public constant onlyAuthorized returns(address,int,string,string,uint,uint) {
      if (trackingInfo[trackingNr].length==0) {
        return;
      }
      ShipmentData data = trackingInfo[trackingNr][index];
      return (data.shipperAddr,data.sensorData,data.dataID,data.currentLocation,data.dataBlockNr,data.dataTimestamp);
    }
