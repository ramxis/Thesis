...
{
  //TODO:erro handling
  var formData = JSON.stringify({"amount":trnsTokenForm.TknAmount.value});
  console.log(formData);//FIXME:remove
  var url = "http://localhost:5001/api/1/transfers"+trnsTokenForm.TknAddress.value+trnsTokenForm.partnerAddr.value;
  var xhr = new XMLHttpRequest();
  ...
  xhr.send(formData);
  //document.getElementById('form').submit();
  alert("Transfer Tokens Successfully...");
}
...
function check_tempty()
{
  if (document.getElementById('partnerAddr').value == "" || document.getElementById('TknAddress').value == ""  || document.getElementById('TknAmount').value == "")
  {
    alert("Fill All Fields !");
  }
  else
...
function div_trasnferTokenForm()
{
