...
library Helper {
  ...
  //helper library for post quantum cryptographic functions
    ...
    function encryptStr(string input,string key,string seed) public constant returns (string cyphertext,string decrypted){
        bytes32 _cyphertext = stringToBytes32(input) ^ keccak256(key,seed);
        string memory result = bytes32ToString(_cyphertext);
        string memory dresult = decryptStr(result,key,seed);
...
contract ShipmentTracker {
    ...
    */
    ...
    public onlyAuthorized(trackingNr) onlyifSigMatch(signature,_seed,_shipperID,trackingNr) {
        ...
        //emit ShippingEvent(msg.sender,_shipperID,data,dataID,location,trackingNr,_status,block.number,block.timestamp); //got stack too deep
    ...
    function decryptTrackingData(uint data, string dataID, string location, string trackingNr, string _status, string _shipperID, string _seed) internal returns(ShipmentData d) {
        ...
        ShipmentData memory _trackingData = ShipmentData(msg.sender,_shipperID,_data,_dataID,_location,__status,block.number,block.timestamp);
    ...
    /*function LogTrackingInformation(uint data, string dataID, string location, string trackingNr, string _status) public onlyAuthorized(trackingNr)  {
     ...
     function LogTrackingInformationNew(uint data, string dataID, string location, string trackingNr, string _status, string _shipperID, bytes32 signature, string _seed) public onlyAuthorized(trackingNr) onlyifSigMatch(signature,_seed,_shipperID,trackingNr){
          if(!chkTrackingNrExists(trackingNr)) {
          addTrackingNumber(trackingNr);
        }
        ShipmentData memory trackingData = addTrackingViolation(data,dataID,location,trackingNr,_status,_shipperID,_seed);
        //ShipmentData memory trackingData = getTrackingData(data,dataID,location,trackingNr,_status,_shipperID,signature,_seed);
        trackingInfo[trackingNr].push(trackingData);
        ChkViolations(trackingData,trackingNr);
  ...
  */
  ...
  function getIpfsAddress(string _trackingNr) public constant onlyAuthorized(_trackingNr) returns(string) { //may be this needs to be without modifiers
    return trackingNrList[_trackingNr].ipfsLogAddr;
  }

  function addTrackingViolation(uint data, string dataID, string location, string trackingNr, string _status, string _shipperID, string _seed) internal returns(ShipmentData d) {
